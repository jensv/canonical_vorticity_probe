...
def mach_results(shot=1210818030, nsmooth=None, res='mach',
    ...
    vtype='octahedron', imin=5E-4, k=1.1, ret_gain=10.0):
    ...
    for i in range(len(tip_abc_name)):
        ...
        if abc == 'ret':
            isat[abc] *= ret_gain
            # Since gain is different for this channel
    ...
    if res.startswith('rat'):
        return {'rat':ratio, 't':time, 'name':rat_name}
    log = - 1.0/k*np.log(ratio) 
    if res.startswith('log'):
        return {'log':log, 't':time, 'name':rat_name}
...
def plot_mach_results(shot=1210818030, nsmooth=None, res='mach',
    ...
    if do_plot:
        mp.clf()
        mp.suptitle(', '.join([
            ' '.join([vtype.capitalize(), 'Mach probe']),
            res, str(shot)]))
        for i, p in enumerate(plot_name):
            ...
            #mp.ylabel(plot_ylabel)
                # Omit for now since isat return gain
                # is not accounted for in 'V'.
